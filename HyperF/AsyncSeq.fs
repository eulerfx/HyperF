...
//        async { return! agent.PostAndAsyncReply(id) }
    ...
    /// Interleaves two async sequences of the same type.
    let rec merge (a:AsyncSeq<_>) (b:AsyncSeq<_>) : AsyncSeq<_> = async {       
        let! a = a        
        match a with
        | Cons (a1, t1) -> return Cons (a1, merge b t1)
        | Nil -> return! b }

    
    /// Projects elements of an async sequence onto a sequence, flattening the results.
    let rec collectSeq (f:'a -> #seq<'b>) (s:AsyncSeq<'a>) : AsyncSeq<'b> = asyncSeq {
        let! s = s
        match s with
        | Cons (head,tail) ->            
            for item in f head do
                yield item            
            yield! collectSeq f tail
        | Nil -> () }

    /// Concatenates an async sequence of sequences.
    ...
    /// Iterates an async sequence invoking each action asynchronously.
    let iterAsyncPar (f:'a -> Async<unit>) (s:AsyncSeq<'a>) : Async<unit> = async {        
        ...
        return! cde.WaitHandle |> Async.AwaitWaitHandle |> Async.Ignore }
    

    /// Iterates an async sequence invoking each action asynchronously.
    let iterAsyncParBatch (f:'a -> Async<unit>) (s:AsyncSeq<#seq<'a>>) : Async<unit> = iterAsyncPar (Seq.map f >> Async.Parallel >> Async.Ignore) s
